...
compilePedigree <-function(pedData)
{
  ...
  pedigree <- data.frame(ID = character(),
                         famID = numeric(),
                         paternalID = numeric(),
                         maternalID = numeric(),
                         sexID = numeric(),
  ...
  for(i in 1:length(uniqueID))
  {
      ...
      subset <- unique(pedData[which(pedData[,"ID"] == uniqueID[i]),]) #isolate subset of file entries sharing same ID
      famSubset <- unique(pedData[which(pedData[,"familyID"] == uniqueID[i]),])
      
      ID <- uniqueID[i] #the ID of the individual entry is iterator value
  ...
  colnames(pedigree) <- c("famID","ID","paternalID","maternalID","sexID","affectionID")
  return(pedigree)
}
